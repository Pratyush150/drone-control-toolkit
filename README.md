...
Every figure below is the unedited output of an example script in `examples/`,
regenerated by running it.

![Step response and controller command for four PID tunes on the same plant](docs/screenshots/pid-tuning-comparison.png)
Four tunes of the same loop in the `sim` harness with a noisy sensor, 15 ms transport delay and 25 ms motor lag: filtering the derivative leaves tracking identical (ITAE 0.673 vs 0.668) and cuts command jitter 17x, from `examples/pid_tuning_comparison.py`.

![Roll estimate, estimation error and gyro bias for a complementary filter and an EKF](docs/screenshots/attitude-complementary-vs-ekf.png)
A complementary filter and an EKF tracking roll on 60 s of simulated IMU data with a 2 deg/s gyro bias and a lateral-acceleration manoeuvre: the complementary filter parks at bias x tau, the EKF makes the bias a state and drives the error to zero, from `examples/complementary_vs_kalman.py`.

![Gyro trace, spectrum and phase lag for low-pass and notch filters](docs/screenshots/vibration-notch-filter.png)
A simulated gyro carrying a 300-400 Hz motor tone: an RPM-tracked notch removes the tone at almost no phase cost near the rate-loop crossover, where the 40 Hz low-pass that also removes it costs about 45 degrees of phase margin, from `examples/vibration_notch_filter.py`.

![Minimum-jerk versus trapezoidal reference, tracking error and a spline waypoint path](docs/screenshots/min-jerk-trajectory.png)
Minimum-jerk, trapezoidal and step references driven through the same closed loop, with a cubic-spline waypoint path whose velocity and acceleration limits are checked as a vector norm across axes, from `examples/min_jerk_trajectory.py`.
